...
const MODEL_PATH = process.env.hostName+'model/rotten/model.json';
console.debug(process.env.hostName+'rotten/model.json')
...
processImage = async (image) => {
    return tfnode.tidy(() => image.expandDims(0).toFloat().div(127).sub(1));
  }
...
classifyLocalImage = async (buffer) => {
    ...
    const model = await tfnode.loadLayersModel(MODEL_PATH);  
    const image = tfnode.tidy( () => tfnode.node.decodeImage(buffer))
